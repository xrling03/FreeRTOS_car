...
#黄色区域色块阈值：
saidao_lab = [(67, 76, -11, 7, 3, 22)]
#物料阈值：
wuliao_lab = [(15, 63, 27, 68, 21, 52),
              (23, 82, -58, -26, 22, 50),
              (10, 68, 8, 39, -71, -23)]
#物料阈值：
...
def camera_Init():
    cam = camera.Camera(512, 320,fps=80)
...
def GetLine(img, comm):
    global LineData
    # LineData=[0,0,0,0]
    line_blob0 = img.find_blobs(saidao_lab, pixels_threshold=500)
    for blob in line_blob0:
        img.draw_rect(blob[0], blob[1], blob[2], blob[3], color=image.COLOR_GREEN)       #圈出巡线区块
        img.draw_cross(blob.cx(), blob.cy(),image.Color.from_rgb(0, 255, 0), size=15, thickness=1)    #圈出中心十字
...
def GetYuan(img, comm):
    ...
    center_y=0
    # Yuanxy=[0,0,0,0] 
    c=img.find_circles(threshold=4400, x_margin=2, y_margin=2, r_margin=2,r_min=20, r_max=100, r_step=2)
    '''
    for c1 in c:
    ...
    if c:
        ...
        for circle in c:
            ...
            center_y += circle.y()
            # print(circle.r())
            img.draw_circle(int(circle.x()), int(circle.y()), int(circle.r()), color=image.COLOR_YELLOW)  # 标注圆
            count += 1
        center_x /= count
        center_y /= count
        # img.draw_circle(int(center_x), int(center_y), 2, color=image.COLOR_WHITE)
        img.draw_cross(int(center_x), int(center_y), image.Color.from_rgb(255, 255, 0), size=20, thickness=1)
...
def GetYuan2(img, comm):
    ...
    global Yuanxy2
    # Yuanxy=[0,0,0,0]
    c=img.find_circles(threshold=5400, x_margin=2, y_margin=2, r_margin=2,
...
def main():
    ...
    while(True):
        ...
        img = cam.read()#读取一张图片
        img.draw_string(2, 2, "fps:%.2f " % (1000.0/t2), image.Color.from_rgba(255, 255, 0, 0.8))
        img.draw_string(120, 2, "模式0:初始化", image.Color.from_rgba(255, 255, 0, 0.8))
        ...
        if Mode_dir==9:
            while(True):
                ...
                disp.show(img)  
                if Mode_dir == 1 or Mode_dir == 2 or Mode_dir == 3 or Mode_dir == 4 or Mode_dir == 5:
                    break
                Mode_dir = comm.read_detect()   
