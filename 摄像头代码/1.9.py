...
def GetYuan(img, comm):
    ...
    center_y=0
    # Yuanxy=[0,0,0,0] 
    c=img.find_circles(threshold=6000, r_margin=5,r_min=20, r_max=22, r_step=1)
    '''
    for c1 in c:
...
def GetYuan2(img, comm):
    ...
    global Yuanxy2
    # Yuanxy=[0,0,0,0]
    c=img.find_circles(threshold=7000, x_margin=2, y_margin=2, r_margin=2,
                      r_min=20, r_max=22, r_step=2)
    '''
    for c1 in c:
        img.draw_circle(c1.x(), c1.y(), c1.r(), color=image.COLOR_WHITE)
        img.draw_circle(c1.x(), c1.y(), 2, color=image.COLOR_WHITE)
